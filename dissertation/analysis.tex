...
\begin{equation}
	N_{A/D} =
		\frac{ N_0^D \cdot \text{lt}^D \cdot T^D(\xi)}{N_0^A \cdot \text{lt}^A \cdot T^A(\xi) } \cdot 
		\frac{ 2 \cdot n_D \cdot L^D }{ A \cdot n_A \cdot L^A } \cdot 
...
This section will discuss the target-to-target relative normalization from the determination of beam intensity, target thickness, live time, tracking efficiency, chamber efficiency, and acceptance. Beam intensity and live time information was obtained from the BIM and scaler data. Tracking efficiencies were extracted from a study combining clean Monte Carlo with actual data noise from the experiment. Acceptance was calculated with Monte Carlo simulation. An isoscalar correction factor is described for cases of asymmetric ($A\neq Z$) nuclei. Sources of combinatorial $\mu^+ \mu^-$ backgrounds from unrelated single muons is studied and subtracted from the data. Finally, a discussion of the effects of deuterium target contamination is and its correction described.
...
\end{equation}
where $\rho_A$ is the density of the material, $\text{NIL}_A$ is the nuclear interaction length of the target, and $\lambda = \frac{\text{NIL}_A}{\rho_A}$. When integrating over the length of the target, one arrives at the \emph{effective} number of protons incident on the target
\begin{equation}
N_{\text{eff}} = N_0 \left[\frac{1 - e^{-\xi}}{\xi}\right] = N_0 T(\xi)\ \ \ ;\ \ \ 
\xi = L/\lambda.
\end{equation}
...
The number of nucleons that the effective protons will interactive should be considered when performing the target-to-target normalization. Aside from the background ``targets'' and liquid hydrogen, there are four nuclear targets in this A-dependence study. The nucleons in liquid deuterium are viewed as essentially free particles, and therefore proved an isoscalar reference by which the heavier targets are compared to. In all, the nuclear targets used are $D_2$, C, Fe, and W.

\begin{table}
\begin{tabular}{lrrrrrrrr}
	\toprule
	{} &  Target Position &  Length[cm] &  $\rho$ [g/cm$^3$] &  NIL [g/cm$^2$] &   $\lambda$ [cm] &  & $\xi$ & T($\xi$) \\
	Target &            &             &                  &              &             &         & &         \\
	\midrule
	$D_2$       &          3 &    50.80 &           0.163 &         71.8 &  439.412485 &        47.973498 \\
	Fe        &          5 &     1.91 &           7.874 &        132.1 &   16.776734 &         1.800824 \\
	C         &          6 &     3.32 &           1.802 &         85.8 &   47.613762 &         3.209060 \\
	W         &          7 &     0.95 &          19.300 &        191.9 &    9.943005 &         0.908300 \\
	\bottomrule
\end{tabular}
