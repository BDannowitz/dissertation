...
In each of these equations~\ref{eq:DY-cross}, \ref{eq:dy-cs-observe}, and \ref{eq:dy-cs-dimensionless}, the cross sections can be factored into two parts: one subprocess cross section and one part that has only a dependence on the parton distribution functions. They are independent of each other, because one of the staples of the quark parton model is that the PDFs ($q(x)$ and $\bar{q}(x)$) are independent of the process by which they are probed.

\subsection{The Quark Parton Model and PDFs}

In the context of hadron-hadron collisions, one can interpret the PDFs as follows: consider two hadrons A and B colliding; a parton of type \emph{a} ($a\in \{u, d, s, g, ...\}$) comes from A and carries with it a fraction of A's momentum ($x_A$).  The same goes for hadron B; a parton of type \emph{b} comes from B and carries momentum fraction $x_B$. Now, the probability of finding the discussed parton from A is given by $q_{a/A}(x_A)dx_A$. Likewise, the probability of finding the discussed parton from B is $q_{b/B}(x_B)dx_B$.

Due to the complex nature of lattice QCD simulations, the parton distributions $q_f(x)$ and $\bar{q}_f(x)$ within the nucleus are determined empirically, with only a few rules based in theory. The primary probe used to measure them has been deep inelastic scattering (DIS), examining the inclusive jet production in order to get a clearer picture of nucleon structure. It should be noted, however, that there have been some Drell-Yan experiments (E605 and E866) that have contributed to these models. The current status of the experimental determination of these PDFs is led by CTEQ (Coordinated Theoretical-Experimental Project on QCD) and TEA (\red{which stands for???}), and are illustrated in Figures~\ref{fig:pdf-q2}~and~\ref{fig:pdf-q100}.

\red{Add DIS diagram and DIS breakdown here?}

Since the photon mediating the DIS interaction is only interacting with one quark (and not two, from two nucleons as in the case of DY), analysis of the scattering allows for a determination of the momentum fraction distribution of the quarks. These distributions, however, are not directly measured. What is actually measured is the $F_2$ structure function, which is a summation over quark flavors. This structure function can be expressed in terms of the quark PDFs as
\begin{equation}
F_2(x) = \nu W_2(x) = \sum_f e_f^2 x [q_f(x) + \bar{q}_f(x)]
\end{equation}
However, due to the universality and process-independence of the PDFs, the distributions measured by DIS must also stand for DY processes.
...
In general, experimental data measuring $F_2$ is used in conjunction with some general constraints in order to arrive at the calculated PDFs. One constraint is to consider the known number of valence quarks in a nucleon. Since the function $q_f(x)$ is the number density of quark flavor $f$ as a function of momentum fraction $x$, the value $q_f(x)dx$ represents the number of quarks with flavor $f$ with fractional momentum in the range of $[x,x+dx]$. Therefore, the known number of valence quarks of flavor $f$ ($N_f$) provides the following condition.
\begin{equation}
...
Looking to Figure~\ref{fig:pdf-q2-q100} we see that at $x>0.1$, $u$ and $d$ quarks dominate $\bar{u}$ and $\bar{d}$ quarks. This is key for the SeaQuest DY measurements due to the kinematic space explored where there is predominantly high $x_1$ and lower $x_2$ observed. In this regime, the Drell-Yan process is probabilistically dominated by a quark from the beam annihilating with an anti-quark from the target. All anti-quarks that exist in the target nucleons must come from what are called the \emph{sea quarks}, or the virtual $q\bar{q}$ pairs 
%
...
\red{Give rough derivation, previous results, a model, and what SeaQuest proposes to provide, stats-wise.}

\subsection{The EMC Effect}

\red{Start from the very beginning with EMC in 1983, discuss current understanding. Draw from review paper and the SeaQuest Academy talks by Don and Arrington.}
