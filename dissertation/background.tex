...
Experimentally, six independent variables are measured, which form a basis by which all eight can be known. This is due to the fact that two pairs of variables, ($M_\gamma$, $x_F$) and ($x_1$, $x_2$) are correlated by the following, ignoring quark masses and considering $p_T << p_{||}$:
\begin{eqnarray}
x_F \def \frac{}{}
\end{eqnarray}
...
\begin{table}[h]
	...
	\begin{tabular}{c|r}
		...
		$x_{1/2}$ & The momentum fraction of the beam/target quark\\
		$M_\gamma$ & The mass of the virtual photon (dimuon)\\
		$x_F$ & The fraction of the max. possible $p_{||}$ carried by virt. photon\\
		$p_T$ & The transverse momentum carried by the virt. photon\\
		$\theta_{\mu}, \phi_{\mu}$ & The polar and azimuthal decay angle\\ & of one of the muons, in the CS ref. frame\\ \hline
...
in which high-mass lepton production occurs as a result of inelastic hadron-hadron collisions \cite{Drell:1970wh}.  This so-called ``Drell-Yan'' (DY) process is identified to be the result of a quark-antiquark annihilation into a virtual photon which then decays into a lepton pair. 
