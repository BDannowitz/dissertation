...
q(x) = q^+(x) + q^-(x)
\end{equation}
and the polarized helicity PDF can be defined as
\begin{equation}
\Delta q(x) = q^+(x) - q^-(x)
\end{equation}
...
\end{eqnarray}
where here $f$ represents all flavors of of quarks and anti-quarks. We see above the relation between $F_1(x)$ and $F_2(x)$ known as the Callan-Gross relation, which stems from 
...
\begin{equation}
x_{tot} = \int_0^1 dx [F_2(x)] \cong \int_0^1 dx [x(q(x) + \bar{q}(x))] \label{eq:mom-sum}
\end{equation}
...
The last interesting sum rule to discuss is the Gottfried sum rule.
%
